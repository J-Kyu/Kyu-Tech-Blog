...
**Key-Point**: Node p remembers last insertion position. If targetNode val is bigger than P's next Node, we start searching from P's next ndoe. But if target Node is smaller, we start searching from head(dummy). This is save lots of time.
...
class Solution(object):
    def insertionSortList(self, head):
        ...
        :rtype: ListNode
        """
        # dummy sorted linked list head node
        dummy = ListNode(0,head)
        # p is the position where cur is going to insert after
        p = dummy
        # cur is node that we compare with
        cur = head 
        
        while cur and cur.next:
            # val represent second node value
            targetNode = cur.next
            targetVal = targetNode.val
            
            # current Node has to be the biggest node, since it is end node of sorted list
            # if we found new node that is bigger than current node, we assume that current node is the second biggest node in the list
            # thus, we change current node to next node. (this will automatically locate previous current node to insertion list)
            if cur.val < targetVal:
                cur = cur.next
                continue
                
            # if p's next value (memorized position node, which represent last position of insertion) is bigger than target Val, we have to search from head
            # we insert dummy to p, so we can find right location from the head of list (avoid from uncovered range)
            # (ex. target val = 4, p = [0(dummy).....5(p),6,7(current Node)....], than we have to start  from dummy  )
            
            # but, if p's next value is less than target value, we can assure that target value will locate between p's next node and before current node.
            # (ex. target val = 4, p = [0(dummy).....3(p),6,7(current Node)....], we can search from p's next node )
            if p.next.val > targetVal:
                p = dummy
            
            # find right location for target Val
            while p.next.val < targetVal:
                p = p.next

            # insertion....located targetNode to ordered position
            tempNode = targetNode
